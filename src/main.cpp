...
#if defined(NDEBUG)
# error "Clementscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x866f26fddb34e25ccce4973d741f4f2ad1f1ddaff75ded671feb8b64c58928ad");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Clementscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Clementscoin Signed Message:\n";
...
{
    // Clementscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 105000); // Clementscoin: 105k blocks in ~6 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.75 * 24 * 60 * 60; // Clementscoin: 18 hours
static const int64 nTargetSpacing = 3.125 * 60; // Clementscoin: 3.125 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Clementscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Clementscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x167ed07be2bf21702f6c0bcf97bb2152bf24ece91f8157e77742317d9d5b98e7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC 02/Mar/2018 Burkina Faso attack: French embassy targeted in Ouagadougou";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 75 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047c78a609591b2cc4dd2a2d3ad73debe19a8d8f541f75a43e7fc691de799fad4009c6114e812f267bb85b77e643ea7b970c54d4093a36d06266afbc93203aeab8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520052689;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085736381;

        if (fTestNet)
        {
            block.nTime    = 1520052672;
            block.nNonce   = 387127380;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x013099d5ac3fea675c08f2b0ec8a493620afa3b65439a78fdc206d5fc9180f7d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc6, 0xbf, 0xd5 }; // Clementscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ClementscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ClementscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ClementscoinMiner(CWallet *pwallet)
{
    printf("ClementscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("clementscoin-miner");
    ...
    try { loop {
        ...
        printf("Running ClementscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ClementscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ClementscoinMiner, pwallet));
}
